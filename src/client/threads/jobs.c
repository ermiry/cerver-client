...
void job_handler_wait (
	...
	if (handler) {
		...
		if (!job_queue_push (job_queue, handler)) {
			...
			while (!handler->done) {
				#ifdef THREADS_DEBUG
				client_log_debug ("job_handler_wait () waiting...");
				#endif
...
static void *job_queue_jobs (void *job_queue_ptr) {
	...
	while (job_queue->running) {
		...
		if (job) {
			#ifdef THREADS_DEBUG
			client_log_debug ("job_queue_jobs () new job!");
			#endif
...
static void *job_queue_handlers (void *job_queue_ptr) {
	...
	while (job_queue->running) {
		...
		if (job_handler) {
			#ifdef THREADS_DEBUG
			client_log_debug ("job_queue_handlers () new job!");
			#endif
			...
			// signal
			#ifdef THREADS_DEBUG
			client_log_debug ("BEFORE job_handler_signal ()");
			#endif

			job_handler_signal (job_handler);

			#ifdef THREADS_DEBUG
			client_log_debug ("AFTER job_handler_signal ()");
			#endif
		}
	}
