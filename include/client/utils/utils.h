...
CLIENT_PUBLIC void c_string_copy (char *to, const char *from);

// copies n bytes from a c string into another one previuosly allocated
CLIENT_PUBLIC void c_string_n_copy (char *to, const char *from, size_t n);
...
CLIENT_PUBLIC size_t c_string_count_tokens (const char *original, const char delim);
...
CLIENT_PUBLIC char **c_string_split (const char *original, const char delim, size_t *n_tokens);
...
CLIENT_PUBLIC void c_string_remove_char (char *string, char garbage);

// removes the exact sub string from the main one
// returns a newly allocated copy of the original str but withput the sub
CLIENT_PUBLIC char *c_string_remove_sub (char *str, const char *sub);
...
CLIENT_PUBLIC bool c_string_starts_with (const char *str, const char *substr);
...
CLIENT_PUBLIC char *c_string_create_with_ptrs (char *first, char *last);
...
CLIENT_PUBLIC char *c_string_remove_sub_after_token (char *str, const char token, char **sub);
...
CLIENT_PUBLIC char *c_string_remove_sub_after_token_with_idx (char *str, const char token, char **sub, int idx);
...
CLIENT_PUBLIC char *c_string_remove_sub_simetric_token (char *str, const char token, char **sub);
...
// returns a newly allocated string, and a option to get the substring
CLIENT_PUBLIC char *c_string_remove_sub_range_token (char *str, const char token, unsigned int first, unsigned int last,
	char **sub);
