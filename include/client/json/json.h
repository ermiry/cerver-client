#ifndef _CLIENT_JSON_H_
#define _CLIENT_JSON_H_
...
CLIENT_PUBLIC enum json_error_code json_error_code (
	const json_error_t *e
);

CLIENT_PUBLIC void jsonp_error_init (
	json_error_t *error, const char *source
);

CLIENT_PUBLIC void jsonp_error_set_source (
	json_error_t *error, const char *source
);

CLIENT_PUBLIC void jsonp_error_set (
	json_error_t *error, int line, int column, size_t position,
	enum json_error_code code, const char *msg, ...
);

CLIENT_PUBLIC void jsonp_error_vset (
	json_error_t *error, int line, int column, size_t position,
...
CLIENT_PUBLIC json_t *json_pack (
	const char *fmt, ...
) CLIENT_ATTRS((warn_unused_result));

CLIENT_PUBLIC json_t *json_pack_ex (
	json_error_t *error, size_t flags,
	const char *fmt, ...
) CLIENT_ATTRS((warn_unused_result));

CLIENT_PUBLIC json_t *json_vpack_ex (
	json_error_t *error, size_t flags,
...
CLIENT_PUBLIC int json_unpack (
	json_t *root, const char *fmt, ...
);

CLIENT_PUBLIC int json_unpack_ex (
	json_t *root, json_error_t *error, size_t flags,
	const char *fmt, ...
);

CLIENT_PUBLIC int json_vunpack_ex (
	json_t *root, json_error_t *error, size_t flags,
	const char *fmt, va_list ap
);
...
CLIENT_PUBLIC void json_print (json_t *root);
